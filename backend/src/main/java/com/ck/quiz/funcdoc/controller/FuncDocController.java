package com.ck.quiz.funcdoc.controller;
 ...
 */
@RestController
@RequestMapping("/api/func/doc")
@RequiredArgsConstructor
@Tag(name = "文档管理", description = "文档管理相关API")
public class FuncDocController {

    private final FuncDocService funcDocService;
    ...
    public ResponseEntity<FuncDocDto> createDocInfo(
            @Valid @RequestBody FuncDocCreateDto createDto) {
        FuncDocDto docInfo = funcDocService.createDocInfo(createDto);
    ...
    public ResponseEntity<Void> deleteDocInfo(
        ...
        funcDocService.deleteDocInfo(id);
    ...
    public ResponseEntity<FuncDocDto> getDocInfoById(
            @Parameter(description = "文档ID") @PathVariable String id) {
        FuncDocDto docInfo = funcDocService.getDocInfoById(id);
        return ResponseEntity.ok(docInfo);
    }
    ...
    public ResponseEntity<Page<FuncDocDto>> pageDocInfo(
            @Valid FuncDocQueryDto queryDto) {
        Page<FuncDocDto> docInfoPage = funcDocService.pageDocInfo(queryDto);
    ...
    public ResponseEntity<Page<FuncDocProcessNodeDto>> pageDocProcessNode(
            @Parameter(description = "文档ID") @PathVariable String docId,
            @Valid FuncDocProcessNodeQueryDto queryDto) {
        // 设置文档ID
        queryDto.setDocId(docId);
        Page<FuncDocProcessNodeDto> processNodePage = funcDocService.pageDocProcessNode(queryDto);
    ...
    public ResponseEntity<FuncDocDto> uploadDocFile(
            @Parameter(description = "文档文件", required = true)
            @RequestParam("file") MultipartFile file) {
        FuncDocDto docInfo = funcDocService.uploadDocFile(file);
        return ResponseEntity.ok(docInfo);
    }
    ...
    public ResponseEntity<List<FuncDocHeadingTreeDto>> getDocHeadingTree(
            @Parameter(description = "文档ID") @PathVariable String id) {
        List<FuncDocHeadingTreeDto> headingTree = funcDocService.getDocHeadingTree(id);
    ...
    public ResponseEntity<List<FunctionPointTreeDto>> getFunctionPointTree(
            @Parameter(description = "文档ID") @PathVariable String docId) {
        List<FunctionPointTreeDto> functionPointTree = funcDocService.getFunctionPointTree(docId);
    ...
    public ResponseEntity<Page<FunctionPointTreeDto>> getThreeLevelFunctionPointsPage(
        ...
        Page<FunctionPointTreeDto> functionPointsPage =
                funcDocService.getThreeLevelFunctionPointsPage(queryDto);
    ...
    public ResponseEntity<Map<String, Object>> generateProcessDescription(
        ...
        Map<String, Object> result = funcDocService.generateByProcess(functionId);
    ...
    public ResponseEntity<String> generateFlow(
            @Parameter(description = "功能点ID", required = true)
            @PathVariable String functionId) {
        String result = funcDocService.generateFlowByProcess(functionId);
        return ResponseEntity.ok(result);
    }
    ...
    public ResponseEntity<Map<String, Object>> generateInf(
            @Parameter(description = "功能点ID", required = true)
            @PathVariable String functionId) {
        Map<String, Object> result = funcDocService.generateInfByProcess(functionId);
        return ResponseEntity.ok(result);
    }
    ...
    public ResponseEntity<byte[]> exportHeadings(
            @Parameter(description = "文档ID") @PathVariable String id) {
        byte[] docxBytes = funcDocService.exportHeadingsToDocx(id);
    ...
    public ResponseEntity<byte[]> exportInfToExcel(
        ...
        byte[] excelBytes = funcDocService.exportToExcel(id);
