...
public class SubjectServiceImpl implements SubjectService {
    ...
    private void loadQuestionNum(List<SubjectDto> subjects) {
        ...
        HumpHelper.lineToHump(jdbcTemplate.queryForList("select k.subject_id, count(*) num from knowledge k inner join question_knowledge_rela r on k.knowledge_id = r.knowledge_id inner join question q on q.question_id = r.question_id where k.subject_id in (:subjectIds) group by k.subject_id", params)).forEach(map -> {
