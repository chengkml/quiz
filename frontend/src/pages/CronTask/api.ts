...
export const getCronTaskList = async (params: CronTaskListParams): Promise<PageResponse<CronTaskDto>> => {
  const response = await axios.get('/cron/task/getCronTaskList', { params });
  return response;
};

// 删除定时任务
export const deleteCronTask = async (ids: string[]): Promise<boolean> => {
  const response = await axios.post('/cron/task/delete', ids);
  return response;
};

// 保存定时任务
export const saveCronTask = async (cronTaskDto: CronTaskDto): Promise<CronTaskDto> => {
  const response = await axios.post('/cron/task/save', cronTaskDto);
  return response;
};

// 触发定时任务
export const triggerCronTask = async (id: string): Promise<boolean> => {
  const response = await axios.post(`/cron/task/trigger/${id}`);
  return response;
};

// 获取任务类型选项
export const getTaskOptions = async (): Promise<any[]> => {
  const response = await axios.get('/cron/task/options');
  return response;
};
