...
import { getDueToday, reviewCharacter, CharacterCardDto } from './api';
...
const ReviewPage: React.FC<ReviewPageProps> = ({ embedded = false, onExit }) => {
    ...
    const [cards, setCards] = useState<CharacterCardDto[]>([]);
                ...
                <div className="review-card-container">
                    <Card className={`review-card ${isFlipped ? 'flipped' : ''}`} onClick={handleFlip}>
                        <div className="card-face card-front">
                            <div style={{ fontSize: 44, fontWeight: 700, marginBottom: 8 }}>{currentCard.characterText}</div>
                            {currentCard.pinyin ? <div style={{ fontSize: 18, color: '#666' }}>{currentCard.pinyin}</div> : null}
                        </div>
                        <div className="card-face card-back">
                            <div className="md-content">
                                <ReactMarkdown remarkPlugins={[remarkGfm]}>
                                    {currentCard.mdDefinition || '无释义'}
                                </ReactMarkdown>
                            </div>
                        </div>
                    </Card>
