...
import { formatDate, formatRelativeTime } from '@/components/DataManager/utils';
...
const AdvancedExample: React.FC = () => {
  ...
  const formConfig: FormFieldConfig[] = [
    ...
    },
    {
      ...
      rules: [
        ...
        { pattern: /^[A-Z0-9-]+$/, message: 'SKU只能包含大写字母、数字和连字符' },
      ],
    },
    {
      field: 'description',
      ...
      content: (
        <Form form={Form.useForm()[0]} layout="vertical">
          {formConfig.slice(4, 6).map((field) => (
            <Form.Item key={field.field} field={field.field} label={field.label}>
    ...
    },
    {
      ...
      render: (value: number) => (
        <Badge
          count={value}
          style={{
            backgroundColor: value > 50 ? '#52c41a' : value > 0 ? '#faad14' : '#f5222d',
          }}
        />
      ),
    },
  ...
  const filterContent = (
    ...
    >
      <Row gutter={16}>
        <Col span={5}>
          <Form.Item field="name" label="产品名称">
            <Input
              placeholder="搜索产品"
              allowClear
              onChange={(value) => {
        ...
        <Col span={9} style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'flex-end' }}>
          <Space>
            <Button
              type="primary"
              icon={<IconSearch />}
  ...
  // 自定义卡片渲染
  const renderCustomCard = (item: Product, index: number, actions: CardActions) => (
    <ArcoCard
      hoverable
      style={{
        height: '100%',
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
        ...
        <Badge
          count={item.stock}
          style={{
            backgroundColor: item.stock > 50 ? '#52c41a' : item.stock > 0 ? '#faad14' : '#f5222d',
          }}
        />
      </div>
